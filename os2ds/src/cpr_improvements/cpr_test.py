...
rules = [
    ...
    (
        CPRRule(modulus_11=True, ignore_irrelevant=False, examine_context=True,
                blacklist=None),
        [matches[i] for i in [0,1,2,3,5,6,19]],
        "match using context rules"
    ),
    (
        CPRRule(modulus_11=True, ignore_irrelevant=False, examine_context=True,
                blacklist=None, whitelist=None),
        [matches[i] for i in [0,2,3,5,6,19]],
    ...
    (
        CPRRule(modulus_11=True, ignore_irrelevant=False, examine_context=True,
                blacklist=None, whitelist=["anders", "and"]),
        [matches[i] for i in [0,1,2,3,4,5,6,19]],
        "match setting `whitelist=['anders', 'and']`"
    ),
]

for rule, matches, description in rules:
    print(description)
    @timing
    def f(rule):
    ...
    ret = f(rule)
    for r, x in zip(ret, matches):
        assert x == r, "oh no something went wrong"
    print(len(ret))
